    ...
    ThemifyConstructor = {
        ...
        binding(_this, data, val, context) {
            ...
            if (logic) {
                ...
                if (context === und || context === null || context.length === 0) {
                    context = _this.closest('.tb_tab,.tb_expanded_opttions') || self.getEl('tb_lightbox_container');
                }
        ...
        tooltip: {
            ...
            addOrRemoveTooltip(show) {
                ...
                const val = self.getEl('_tooltip').value;
                ...
                if (val !== '') {
                    ...
                    let width = self.getEl('_tooltip_w').value;
                    tooltip.classList.add('tooltip_preview');
                    tooltip.innerHTML = val;
                    tooltip.style.background = api.Helper.getColor(self.getEl('_tooltip_bg'));
                    tooltip.style.color = api.Helper.getColor(self.getEl('_tooltip_c'));
                    if (width !== '') {
                        width += self.getEl('_tooltip_w_unit').value;
                    }
        ...
        angle: {
            ...
            render(data, self) {
                ...
                const angle = wrap.querySelector('#' + data.id),
                    event=data.event!==und?data.event:(self.clicked === 'styling'? 'keyup' : 'change');
        ...
        lottie:{
                ...
                return {
                    ...
                    options:[
                        ...
                        },
                        {
                            label:lb.csssel,
                            id:'sel',
                            wrap_class:'tb_click_hover_input',
                            type:'text',
                            control:{event:'change'}
                        },
                        {
                            ...
                            comment:lottieLb.clsel
                        },
                        {
                            type:'message',
                            wrap_class:'tb_hover_msg',
                            label:'',
                            comment:lottieLb.clhov
                        },
                        {
                            id: 'count',
                            label: lb.count,
            ...
            render(data, self) {
                ...
                browse.tfOn(Themify.click,async e=>{
                    ...
                    .textContent=themifyBuilder.i18n.lottie_lib.browse;
        ...
        export_lottie:{
            render(data,self){
                ...
                exportBtn.tfOn(Themify.click,e=>{
                    e.stopPropagation();
                    ...
                    const html='<tf-lottie data-lazy="1" class="tf_w tf_lazy"><template>'+JSON.stringify(api.Helper.clear(opt))+'</template></tf-lottie>',
                        msg=self.label.lottie.copy.replaceAll('%html%','<textarea style="border:0;outline:0;resize:none;display:inline-table;font-weight:bold;margin-top:20px" readonly class="tf_w">'+html+'</textarea>');
                    api.LiteLightBox.alert(msg);
                    const textarea=api.LiteLightBox.el.querySelector('textarea'),
                        select=()=>{
                            textarea.focus();
                            textarea.select();
                        };
                    ...
                    .textContent=self.label.lottie.exp;
                return exportBtn;
            }
        },
        icon_radio: {
                ...
                const f = doc.createDocumentFragment(),
                    ...
                    callback = () => {
                        ...
                        datePicker.call($(input), {
                            showTimepicker: !!(data.timepicker === und || data.timepicker),
        ...
        position_box: {
            ...
            getLabel(val) {
                let pos,
                    lb=ThemifyConstructor.label;
                switch (val) {
                    case '0,0':
                        pos = lb.top+' '+lb.left;
                        break;
                    case '50,0':
                        pos = lb.top+' '+lb.center;
                        break;
                    case '100,0':
                        pos = lb.top+' '+lb.right;
                        break;
                    case '0,50':
                        pos = lb.center+' '+lb.left;
                        break;
                    case '50,50':
                        pos =  lb.center+' '+lb.center;
                        break;
                    case '100,50':
                        pos = lb.center+' '+lb.right;
                        break;
                    case '0,100':
                        pos = lb.bottom+' '+lb.left;
                        break;
                    case '50,100':
                        pos = lb.bottom+' '+lb.center;
                        break;
                    case '100,100':
                        pos = lb.bottom+' '+lb.right;
                        break;
                    default:
            ...
            render(data, self) {
                ...
                for (let i in positions) {
                    ...
                    tooltip.className = 'themify_tooltip';
                    span.textContent = this.getLabel(positions[i]);
                ...
                if(!data.wrap_class && data.prop==='background-position'){
            ...
            render(data, self) {
                ...
                if (data.opposite === true) {
                    select.tfOn('change', function (e) {
                        e.stopPropagation();
                        self.margin.changeUnit(this,self);
                    }, {passive: true});
                }
        ...
        createMarginPadding(type, data) {
            const labels=this.label,
                options = data.options || [
                    {id: 'top', label: labels.top},
                    {id: 'bottom', label: labels.bottom},
                    {id: 'left', label: labels.left},
                    {id: 'right', label: labels.right}
                ],
                ul = doc.createElement('ul'),
                self = this,
                id = data.id,
        ...
        text_shadow: {
            ...
            render(data, self) {
                const labels=self.label,
                    ...
                    li = doc.createElement('li'),
                    prop_id = id + '_color',
                    color = {id: prop_id, class: 'tb_shadow_field', selector: selector},
                    range = api.Helper.cloneObject( data);
        ...
        filters: {
            getFields(){
                const labels=ThemifyConstructor.label;
                return {
                    hue: {
                        label: labels.hue,
                        units: {deg: {max: 360}},
            ...
            update(id,v,self){
                const ranges= this.getFields(),
                    mid=self.stylesData[id].mid;
            ...
            render(data, self) {
                const sel=data.selector,
                    ranges = this.getFields(),
                    f=doc.createDocumentFragment(),
                    ul = doc.createElement('ul'),
                    mode=self.create([{
                        id:data.mid,
                        type:'select',
                        label:self.label.b_m,
                        prop:'mix-blend-mode',
                        selector:sel,
                        blend:true
                    }]);
        ...
        height: {
            ...
            render(data, self) {
                const units = {
                    ...
                    minId=data.minid || 'mi_h',
                    maxId=data.maxid || 'mx_h';
        ...
        width: {
            ...
            render(data,self,minId,maxId,units) {
                ...
                const autoLabel=prop==='width'?self.label.a_wd:self.label.a_ht,
                    opt=
                        [   {
        ...
        fontSize:{
            ...
            render(data, self,units) {
                ...
                if(!data.label && data.prop==='font-size'){
                    data.label='f_s';
                }
                return res;
            }
        ...
        lineHeight:{
            ...
            render(data, self) {
                ...
                const res=  self.fontSize.render(data,self,units);
                if(!data.label){
                    data.label='l_h';
                }
                return res;
        ...
        fontStyle:{
            update(id,v,self){
                const id2=self.stylesData[id].id2;
        ...
        transform: {
            ...
            render(data, self) {
                const id=data.id,
                    ...
                    prop=data.prop,
                    labels=self.label,
                    className='tb_transform_field'+(data.class?' '+data.class:''),
                    options= [
                        {id: 'top', label: 'X'},
                        {id: 'bottom', label: 'Y'}
                    ],
                    transforms=[
                        {
                            type:'group',
                            wrap_class:'tb_transform_fields tf_w'+(data.wrap_class?' '+data.wrap_class:''),
                            label:'',
                            options:[
                                {
                                    id:id+'_scale',
