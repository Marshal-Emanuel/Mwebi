...
(function (win, doc,und) {
    ...
    ThemifyStyles = {
        ...
        parseFontName(font) {
            ...
            if (font) {
                font = font.split(',');
                for (let i = 0, len = font.length; i < len; ++i) {
                    let v = font[i].trim();
            ...
            if(id){
                ...
                if (bp === 'desktop') {
                    ...
                    if (data['breakpoint_' + bp] !== und && data['breakpoint_' + bp][id] !== und && data['breakpoint_' + bp][id] !== '') {
                        ...
                        for (let points = this.breakpointsReverse,i = points.indexOf(bp) + 1, len = points.length; i < len; ++i) {
                            if (points[i] !== 'desktop') {
        ...
        createCss(data, elType, saving, gsClass, isGSCall, optimize) {
            ...
            if (isGSCall === und) {
                ...
                if (inlineFonts) {
                    const oldBp = this.breakpoint;
                    this.breakpoint = 'desktop';
                    for (let i = inlineFonts.length - 1; i > -1; --i) {
                        let f = inlineFonts[i].replace(/font-family:|;|\'/ig, '').trim();
                        if (f) {
                            let tmp = {};
                            tmp[i] = f;
                                ...
                                for (var i in styles) {
                                    if (styles[i] !== null) {
                                        ...
                                        if (type === 'expand' || type === 'multi' || type === 'group') {
                                            let p = parent;
                                            if (type === 'expand' && styleData.label !== und) {
                                                p = styleData.label.replace(/\s/g, '');
                                                if (parent !== und) {
                                                    p = parent + '_' + p;
                                                }
                                            }
                                            getStyles(styleData.options, p);
                                        } else if (type === 'tabs') {
                                            for (let j in styleData.options) {
                                                let p = parent === und?j:(parent + '_' + j);
                                                getStyles(styleData.options[j].options, p);
                                            }
                                        } else if (styleData.id !== und) {
                                            let id = styleData.id;
                                            if (styleData.prop !== und) {
                                                Rules[module][id] = this.getStyleData(styleData, parent);
                                                let prop = styleData.prop;
                                                ...
                                                if (type === 'box_shadow' || type === 'text_shadow') {
                                                    let vals = type === 'box_shadow' ? ['hOffset', 'vOffset', 'blur', 'spread', 'color'] : ['hShadow', 'vShadow', 'blur', 'color'];
                                                    for (let j = vals.length - 1; j > -1; --j) {
                                                        let k = id + '_' + vals[j];
                                                        Rules[module][k] = this.getStyleData(styleData, parent);
                                                        if (vals[j] !== 'color') {
                                                    ...
                                                    if (Rules[module][styleData.s] === und) {
                                                        Rules[module][styleData.s] = {type: 'color', prop: 'color', isFontColor: true, selector: selector, origId: id, 'p': parent};
                                                    }
                                                    if (Rules[module][styleData.g] === und) {
                                                        Rules[module][styleData.g] = {type: 'gradient', p: parent};
                                                        Rules[module][styleData.g] = Rules[module][styleData.g + '-gradient-angle'] = Rules[module][styleData.g + '-circle-radial'] = Rules[module][styleData.g + '-gradient-type'] = {type: 'gradient', 'p': parent};
                                                    }
                                                } else if (type === 'fontStyle') {
                                                    Rules[module][id+'_regular']=this.getStyleData(styleData, parent);
                                                    Rules[module][id+'_regular'].type='icon_radio';
                                                    Rules[module][styleData.id2]={type: 'icon_radio',prop:'font-weight',selector:selector, p: parent};

                                                }else if (type === 'padding' || type === 'margin' || type === 'border' || type === 'border_radius') {
                                                    let vals = ['top', 'right', 'bottom', 'left'],
                                                    ...
                                                    for (let j = vals.length-1; j > -1; --j) {
                                                        let k = id + '_' + vals[j];
                                                        if (is_border === true) {
                                                            Rules[module][k + '_style'] = this.getStyleData(styleData, parent);
                                                            Rules[module][k + '_color'] = this.getStyleData(styleData, parent);
                                                    ...
                                                    Rules[module][id + '-gradient'] = this.getStyleData(styleData, parent);
                                                    ...
                                                    if (type === 'imageGradient') {
                                                        Rules[module][id + '-type'] = {type: 'radio', p: parent};
                                                        //bg
                                                        Rules[module][styleData.colorId] = this.getStyleData(styleData, parent);
                                                        Rules[module][styleData.colorId].prop = 'background-color';
                                                        Rules[module][styleData.colorId].type = 'color';
                                                        Rules[module][styleData.colorId].id = styleData.colorId;
                                                    ...
                                                    let vals = ['hue', 'saturation', 'brightness', 'contrast', 'invert', 'sepia', 'opacity', 'blur'];
                                                    for (let j = vals.length - 1; j > -1; --j) {
                                                        Rules[module][id + '_' + vals[j]] = this.getStyleData(styleData, parent);
                                                    }
                                                    Rules[module][styleData.mid]={prop:'mix-blend-mode',selector:selector, type: 'select'};

                                                } else if (type === 'width' || type==='height') {
                                                    let minId=type === 'width'?('min_' + id):(styleData.minid || 'mi_h'),
                                                        maxId=type === 'width'?('max_' + id):(styleData.maxid || 'mx_h');
                                                    Rules[module][minId] = {prop: 'min-'+type, selector: selector, type: type, p: parent};
                                                    Rules[module][maxId] = {prop: 'max-'+type, selector: selector, type: type, p: parent};
                                                    Rules[module][id + '_auto_'+type] = {prop: type, selector: selector, type: type, p: parent};
                                                    Rules[module][id + '_unit'] = {type: 'select', p: parent};

                                                } else if (type === 'transform') {
                                                    let t_data = this.getStyleData(styleData, parent);
                                                        ...
                                                        Rules[module][id+'_position']={
                                                            prop:'transform-origin',
                                                            type:'position_box',
                                                            selector:t_data.selector
                                                        };
                                            ...
                                            Rules[module][styleData.topId] = {prop: 'margin-top', selector: selector, type: 'range', p: parent};
                                            Rules[module][styleData.bottomId] = {prop: 'margin-bottom', selector: selector, type: 'range', p: parent};
                                            Rules[module][styleData.topId + '_unit'] = {type: 'select', p: parent};
                                            Rules[module][styleData.bottomId + '_unit'] = {type: 'select', p: parent};
                                            Rules[module][styleData.topId + '_opp_top'] = {type: 'checkbox', p: parent};
                                        }
                                    }
                                }
                            };
            ...
            if (AllFields[module] !== und) {
                ...
                for (let i in settings) {
                    ...
                    if (rules[i] !== und && rules[i].selector !== und) {
                        ...
                        else if (type === 'letterSpace' || type === 'lineHeight' || type === 'fontSize' || type==='zIndex') {
                            type = 'range';
                        }
                        else if (rules[i].style_handler !== und) {
                            type = rules[i].style_handler;
                        }
                        let st = this.fields[type].call(this, i, module, rules[i], settings, elementId, allData);
                        if (st !== false) {
                            let selectors = Array.isArray(rules[i].selector) ? rules[i].selector : [rules[i].selector],
        ...
        fields: {
            ...
            image(id, type, args, data, elementId, allData) {
                let v = this.getStyleVal(id, allData),
                        selector = false;
                if (v !== und) {
                    if (id === 'background_image' || id === 'bg_i_h') {
                        let checked = id === 'background_image' ? 'background_type' : 'b_t_h';
                        checked = this.getStyleVal(checked, allData);
            ...
            icon_radio(id, type, args, data, elementId, allData) {
                const v = this.getStyleVal(id, allData);
            ...
            color(id, type, args, data, elementId, allData) {
                ...
                if (args.colorId === id && args.origId !== und && !this.getStyleVal(args.origId, allData)) {
                    ...
                    const imgId = id === 'b_c_h' ? 'bg_i_h' : 'b_i_i_h';
            ...
            padding(id, type, args, data, elementId, allData) {
                ...
                let unit = this.getStyleVal(id + '_unit', allData) || 'px',
            ...
            box_shadow(id, type, args, data, elementId, allData) {
                ...
                let subSets = prop === 'box-shadow' ? ['hOffset', 'vOffset', 'blur', 'spread'] : ['hShadow', 'vShadow', 'blur'],
                        selector = '',
                        allIsempty = true;
                for (let i = 0, len = subSets.length; i < len; ++i) {
                    let tid = origId + '_' + subSets[i],
            ...
            border_radius(id, type, args, data, elementId, allData) {
                ...
                const unit = this.getStyleVal(id + '_unit', allData) || 'px';
                return prop + ':' + v + unit + '#@#';
            },
            border(id, type, args, data, elementId, allData) {
                ...
                let all = this.getStyleVal(origId + '-type', allData) || 'top';
                if (all === 'all') {
                ...
                let style = this.getStyleVal(id.replace('_width', '_style'), allData),
                ...
                if (style === 'none') {
                    ...
                    val = v + 'px ' + style;
                    let color = this.getStyleVal(colorId, allData);
                    if (color !== '' && color !== und) {
            ...
            select(id, type, args, data, elementId, allData) {
                ...
                if (prop === 'background-mode' || prop === 'background-repeat' || prop === 'background-attachment') {
                    ...
                    let selectedType=suffix?null:this.getStyleVal('background_type', allData);
                    ...
                    if (prop === 'background-mode') {
                        ...
                        if (bg_values[v] !== und) {
                            if (v.indexOf('repeat') !== -1) {
                                ...
                                if (v === 'best-fit-image' || v === 'builder-parallax-scrolling' || v === 'builder-zoom-scrolling' || v === 'builder-zooming') {
                                    let tmp = this.extend(true, {}, args);
                                    tmp.prop = 'background-position';
                                    let pos = this.fields.position_box.call(this, 'background_position'+suffix, type, tmp, data, elementId, allData) ||  'background-position:center#@#';
                                    tmp = null;
                                    selector += pos;
                        ...
                        const bgMode=  this.getStyleVal('background_repeat'+suffix, allData);
                    ...
                    if (v == '0') {
                        let opt = [id, id + '_gap', id + '_divider_color', id + '_width', id + '_divider_style'];
                        for (let i = opt.length - 1; i > -1; --i) {
                            delete data[opt[i]];
                        }
                        return false;
                    }
                    let gap = this.getStyleVal(id + '_gap', allData);
                    if (gap) {
                        selector = 'column-gap:' + gap + 'px#@#';
                    }
                    let style = this.getStyleVal(id + '_divider_style', allData),
            ...
            font_select(id, type, args, data, elementId, allData) {
                let _data = allData ? allData : data,
                ...
                let is_google_font = (typeof ThemifyConstructor !== 'undefined' && ThemifyConstructor.font_select.google[v] !== und) || (typeof ThemifyBuilderStyle !== 'undefined' && ThemifyBuilderStyle.google[v] !== und),
                ...
                if (is_google_font || is_cf_font) {
                    let w = this.getStyleVal(id + '_w', _data),
                    ...
                    if (w) {
                        ...
                        const italic = w.indexOf('italic') !== -1 ? ';font-style:italic' : '';
            ...
            range(id, type, args, data, elementId, allData) {
				...
				v=v.toString();
                let unit = args.prop!=='z-index'?(this.getStyleVal(id + '_unit', allData) || 'px'):'',
            ...
            radio(id, type, args, data, elementId, allData) {
                if (args.prop === 'frame-custom') {
                    ...
                    if (v === side + '-presets') {
                        ...
                        let key = Themify.hash(layout),
                                callback = svg=> {
                                    const color = this.getStyleVal(side + '-frame_color', allData);
                                    if (color !== und && color !== '') {
                                        svg = svg.replace(/\#D3D3D3/ig, this.toRGBA(color));
                                    }
                                    selector = 'background-image:url("data:image/svg+xml;utf8,' + encodeURIComponent(svg) + '")#@#';
                                };
                        if (this.fields.frameCache[key] !== und) {
                            callback(this.fields.frameCache[key]);
                        } else {
                            let frame = doc.tfId('tmpl-frame_' + layout);
                            if (frame !== null) {
                                this.fields.frameCache[key] = frame.textContent.trim();
                                callback(this.fields.frameCache[key]);
                            } else {
                                let url = isVisual !== true && typeof themifyBuilder !== 'undefined' ? themifyBuilder.builder_url : ThemifyBuilderStyle.builder_url,
                                        xhr = new XMLHttpRequest();
                                url += '/img/row-frame/' + layout + '.svg';
                                xhr.open('GET', url, false);
                                xhr.onreadystatechange = () => {
                                    if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
										this.fields.frameCache[key] = xhr.responseText;
                    ...
                    let w = this.getStyleVal(side + '-frame_width', allData),
            ...
            multiColumns(id, type, args, data, elementId, allData) {
                ...
                if (v) {
                    ...
                    if (w) {
                        let s = this.getStyleVal(id + '_divider_style', allData),
            ...
            filters(id, type, args, data, elementId, allData) {
                ...
                selector = '';
                for (let k in ranges) {
                    let v = this.getStyleVal(id + '_' + k, allData);
                    if (v!==und) {
            ...
            number(id, type, args, data, elementId, allData, bp, as_array) {
                return  this.fields.text.call(this, id, type, args, data, elementId, allData, bp, as_array);
            },
            width(id, type, args, data, elementId, allData,bp, as_array) {
                let prop = args.prop,
                    selector = false,
                    v = this.getStyleVal(id, allData);
                if((prop==='width' || prop==='height') && this.getStyleVal(id + '_auto_'+prop, allData)==='auto'){
                    v='auto';
                }
                if (v!==und) {
                    const unit = v==='auto'?'':(this.getStyleVal(id + '_unit', allData) || 'px');
                    selector = prop+':' + v + unit + '#@#';
                }
                return selector;
            },
            height(id, type, args, data, elementId, allData, bp, as_array) {
                return this.fields.width.call(this,id, type, args, data, elementId, allData, bp, as_array);
            },
            position(id, type, args, data, elementId, allData) {
                ...
                if (v!==und) {
                    selector = 'position:' + v + '#@#';
                    if ( v!=='' && v!=='static') {
                        for (let pos = ['top', 'right', 'bottom', 'left'],i = pos.length - 1; i >-1; --i) {
                            let val = this.getStyleVal(id + '_' + pos[i] + '_auto', allData);
                            if (val !== 'auto'){
                                val = this.getStyleVal(id + '_' + pos[i], allData);
                                val = val!==und && !isNaN(val) ? (val + (this.getStyleVal(id + '_' + pos[i] + '_unit', allData) || 'px')) : '';
                            }
                            if(val!==''){
            ...
            transform(id, type, args, data, elementId, allData) {
                ...
                for (let i = options.length-1; i > -1; --i) {
                    ...
                    switch (opt) {
                        ...
                        case 'translate':
                        case 'skew':
                            let x = this.getStyleVal(orig_id + '_' + opt + '_top', allData),
                            y = this.getStyleVal(orig_id + '_' + opt + '_bottom', allData),
                            unit = 'skew' === opt ? 'deg' : '',
                            isTranslate='translate' === opt;
                            if (isTranslate) {
                                unit = {
                                    x: this.getStyleVal(orig_id + '_' + opt + '_top_unit', allData) || 'px',
                                    y: this.getStyleVal(orig_id + '_' + opt + '_bottom_unit', allData) || 'px'
                                };
                            }
                            if (x!==und || y!==und) {
                                if (x && this.getStyleVal(orig_id + '_' + opt + '_opp_bottom', allData)) {
                                    selector += opt + '(' + x + (isTranslate ? unit.x : unit) + ') ';
                                } else if (x!==und && y!==und) {
                                    selector += opt + '(' + x + (isTranslate ? unit.x : unit) + ',' + y + (isTranslate? unit.y : unit) + ') ';
                                } else {
                                    selector += x ? opt + 'X(' + x + (isTranslate ? unit.x : unit) + ') ' : opt + 'Y(' + y + (isTranslate ? unit.y : unit) + ') ';
                                }
                            }
                            break;
                        case 'rotate':
                            for (let inputs = ['z', 'y', 'x'], k = inputs.length-1; k > -1; --k) {
                                v = this.getStyleVal(orig_id + '_' + opt + '_' + inputs[k], allData);
                                if (v) {
                                    selector += opt + inputs[k].toUpperCase() + '(' + v + 'deg) ';
                                }
                            }
                            break;
                    }
            ...
            aspectRatio(id, type, args, data, element_id, allData, bp, as_array) {
                let selector =false,
                    v=this.getStyleVal(id, allData);
                if(v){
                    selector= args.prop+':'+v+'#@#';
                    ...
                    for (i in css) {
                        if (i !== 'gs' && i !== 'bg') {
                            if (i !== 'fonts' && i !== 'cf_fonts') {
                                ...
                                cssText = '';
                                if (i !== 'desktop') {
                                    let w = ThemifyBuilderStyle.points[i];
                                    if (i !== 'mobile') {
                                        w = w[1];
                                    }
                                    st.media = '(max-width:' + w + 'px)';
